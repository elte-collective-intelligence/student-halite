"""Training script for Independent Q-Learning with Hydra configuration."""
...
@hydra.main(version_base=None, config_path="../../configs", config_name="train/train_iql")
def train(cfg: DictConfig):
    """Train Independent Q-Learning on Halite environment."""
    ...
    for episode in range(start_episode, exp_cfg.num_episodes):
        ...
        while not done:
            ...
            # Train step
            update_stats_list = algo.train_step(obs, action, reward, next_obs, done)
        ...
        logger.log_episode_metrics(episode + 1, episode_reward, episode_length, env.state, update_stats_list)
        ...
        if (episode + 1) % exp_cfg.log_frequency == 0:
            ...
            print(f"  Epsilon: {algo.epsilon:.3f}")
            if update_stats_list and len(update_stats_list) > 0:
                # Filter out None values and ensure we have dicts
                valid_stats = [s for s in update_stats_list if s is not None and isinstance(s, dict)]
                if valid_stats:
                    avg_loss = np.mean([s.get('loss', 0) for s in valid_stats])
        ...
        if (episode + 1) % exp_cfg.save_frequency == 0:
            ...
            try:
                player_names = [f"IQL_Agent_{i}" for i in range(env.num_agents)]
