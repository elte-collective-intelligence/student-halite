"""Training script for Centralized Q-Learning with Hydra configuration."""
...
from src.algo.cql import CentralizedQLearning
...
@hydra.main(version_base=None, config_path="../../configs", config_name="train/train_cql")
def train(cfg: DictConfig):
    """Train Centralized Q-Learning on Halite environment."""
    ...
    for episode in range(start_episode, exp_cfg.num_episodes):
        ...
        while not done:
            ...
            # Train step
            update_stats = algo.train_step(obs, action, reward, next_obs, done)
        ...
        if (episode + 1) % exp_cfg.log_frequency == 0:
            ...
            if update_stats:
                print(f"  Loss: {update_stats.get('loss', 0):.4f}")
                print(f"  Q-mean: {update_stats.get('q_mean', 0):.4f}")
        ...
        if (episode + 1) % exp_cfg.save_frequency == 0:
            ...
            try:
                player_names = [f"CQL_Agent_{i}" for i in range(env.num_agents)]
        ...
        if eval_config is not None and (episode + 1) % exp_cfg.eval_frequency == 0:
            ...
            print(f"[Evaluation] Running evaluation at episode {episode + 1}")
