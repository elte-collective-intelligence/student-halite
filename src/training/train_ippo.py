"""Training script for Independent PPO with Hydra configuration."""
...
from src.algo.ippo import IPPO
...
@hydra.main(version_base=None, config_path="../../configs", config_name="train/train_ippo")
def train(cfg: DictConfig):
    """Train Independent PPO on Halite environment."""
    ...
    for episode in range(start_episode, exp_cfg.num_episodes):
        ...
        while not done:
            ...
            # Train step (collects data in episode buffers, returns stats when done)
        ...
        if (episode + 1) % exp_cfg.save_frequency == 0:
            ...
            try:
                player_names = [f"IPPO_Agent_{i}" for i in range(env.num_agents)]
